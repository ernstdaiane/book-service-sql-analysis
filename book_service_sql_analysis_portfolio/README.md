...
### ✍️ Highest-rated author in the qualified sample
...
├── README.md
├── notebook/
│   └── book_service_sql_analysis.ipynb
├── sql/
│   └── book_service_analysis.sql
├── .env.example
└── requirements.txt
```
...
The original project connected to a course database.

**Database credentials are not included in this public portfolio repository.**

The portfolio notebook uses environment variables for connection settings, and the original executed query outputs are retained so the analysis and results can still be reviewed without direct access to the source database.

## About

This project was developed as part of my Data Analytics studies at TripleTen.

My focus was not only on writing SQL queries, but on using relational data to answer clear business questions and communicate the results in a practical way.
